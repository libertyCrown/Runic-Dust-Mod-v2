...
public class InscriptionRespawn extends InscriptionEvent
{

	public InscriptionRespawn(int[][] design, String idName, String properName,
			int id)
	{
	...
	public void onUpdate(EntityLiving wearer, ItemStack item, boolean[] buttons)
	{
				...
				&& tag.hasKey("RespawnX"))
		{
			...
			if (wearer.getDistanceSq(x, y, z) <= 9D)
				return;
	...
	public ItemStack onItemRightClick(EntityPlayer wearer, ItemStack item)
	{
		...
		double dist = (lastX - (double) x) * (lastX - (double) x)
				+ (lastY - (double) y) * (lastY - (double) y)
				+ (lastZ - (double) z) * (lastZ - (double) z);
		...
		item.getTagCompound().setString("description",
				"Set to [" + x + "x, " + y + "y, " + z + "z]");
		if (dist > 1)
			// TODO-fix chat message
			// wearer.sendChatToPlayer("Set return point to [" + x + "x, " + y +
			// "y, " + z + "z]");
