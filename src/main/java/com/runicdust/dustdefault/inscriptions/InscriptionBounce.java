package com.runicdust.dustdefault.inscriptions;
...
public class InscriptionBounce extends InscriptionEvent
{

	public InscriptionBounce(int[][] design, String idName, String properName,
			int id)
	{
	...
	{
		ItemStack[] req = new ItemStack[] { new ItemStack(Item.slimeBall, 8),
			...
			boolean[] buttons)
	{
		...
		if (!wearer.onGround)
		{
			if (getLastYVel(item) > wearer.motionY)
				setFalling(item, true, getFallDist(item) + wearer.fallDistance,
						(float) wearer.motionX, (float) wearer.motionY,
						(float) wearer.motionZ);
	...
	private void setFalling(ItemStack item, boolean val, float dist,
