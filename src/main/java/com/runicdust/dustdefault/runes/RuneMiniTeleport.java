...
import com.runicdust.dustcore.util.VoidTeleManager;

/**
 * 
 * @author billythegoat101
 */
public class RuneMiniTeleport extends DustEvent
{
	public RuneMiniTeleport()
	{
		super();
	}
	...
	public void onInit(EntityDust e)
	{
		...
		// ItemStack[] req = this.sacrifice(e, new ItemStack[]{new
		// ItemStack(Block.obsidian, 2)});
		...
		// Block.blocksList[e.data[0]].getBlockName());
		// }
		// }
		// else
		// {
		...
		// for (int z = -1; z <= 1; z++) {
		// if (x == 0 || z == 0) {
		// if (world.getBlockId(fnd[0] + x, fnd[1], fnd[2] + z) ==
		// mod_DustMod.dust.blockID) {
		// TileEntityDust ted = (TileEntityDust) world.getBlockTileEntity(x +
		// fnd[0], fnd[1], fnd[2] + z);
		// int gamt = 4;
		// int bamt = 4;
		// for (int i = 0; i < 4; i++) {
	...
	public void onTick(EntityDust e)
	{
		...
		// System.out.println("DURR " + e.worldObj.worldProvider.worldType + " "
		// + ents.size());
		...
		for (Object o : ents.toArray())
		{
			...
			if (!(i instanceof EntityDust) && Math.abs(dx) < tol
					&& Math.abs(dz) < tol && Math.abs(dy) < 3D/*
															 * i instanceof
															 * EntityLiving &&
															 * e.
															 * getDistanceToEntity
															 ...
															 */)
			{
				...
				if (i instanceof EntityPlayer
						&& ((EntityPlayer) i).timeUntilPortal < 300)
				{
					e.ram = 100;
				}
				...
				for (int temp = index + 1; temp != index && index != -1; temp++)
				{
					...
					// System.out.println("Found warp: " + warp[3] + ":" +
					// warp[4] + " " + iwarp[3] + ":" + iwarp[4] + " dim:" +
					// iwarp[6] + " ver:" + iwarp[7]);
					if ((Math.abs(warp[0] - iwarp[0]) < 0.5D
							&& Math.abs(warp[1] - iwarp[1]) < 0.5D && Math
							.abs(warp[2] - iwarp[2]) < 0.5D)
							|| iwarp[6] != i.worldObj.provider.dimensionId
							|| iwarp[7] != warp[7])
					{
					...
					// if(ed != null && ed.data == e.data[0] && !ed.equals(e) &&
					// ed != e && ed.ram == 1){
					if (warp[3] == iwarp[3] && warp[4] == iwarp[4])
					{
						// System.out.println("Found warp location " +
						// Arrays.toString(iwarp) + " " +
						// Arrays.toString(warp));
						if (VoidTeleManager.skipWarpTick > 0)
						{
							// mod_DustMod.skipWarpTick -- ;
							// e.ram = 100;
							// System.out.println("Skipping due to recent tele "
							// + mod_DustMod.skipWarpTick);
							break stopWarp;
						}
						...
						{
							((EntityLiving) i).setPositionAndUpdate(
									iwarp[0] + 0.5D, iwarp[1] + 0.6D,
									iwarp[2] + 0.5D);
						} else
						{
							i.setPosition(iwarp[0] + 0.5D, iwarp[1] + 0.6D,
									iwarp[2] + 0.5D);
						}
						...
						// System.out.println("Sending to dimension " +
						// i.worldObj.worldProvider.worldType);
						// System.out.println("new loc " + i.posX + " " + i.posY
						// + " " + i.posZ);
						// System.out.println("DELTA " + dx + " " + dy + " " +
						// dz);
						// e.ram = 100;
						EntityDust enWarp = VoidTeleManager.getWarpEntity(
								iwarp, e.worldObj);
			...
			{
				// System.out.println("Derp? " + (!(i instanceof EntityDust)) +
				// " " + e.getDistanceToEntity(i) + " " + dx + " " + dz + " " +
				// dy);
			}
		}
