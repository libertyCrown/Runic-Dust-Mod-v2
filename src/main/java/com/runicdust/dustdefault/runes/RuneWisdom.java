 ...
 */
public class RuneWisdom extends DustEvent
{
	public RuneWisdom()
	{
		super();
	...
	{
		ItemStack[] req = new ItemStack[] { new ItemStack(Item.ingotIron, 16,
				-1) };
		sacrifice(e, req);
		...
		for (Entity i : ents)
		{
			...
			if (i instanceof EntityPlayer && e.ram <= 0)
			{
				...
				if (p.username.equals(e.summonerUN))
				{
					...
					// System.out.println("Wat " + p.experience + " " +
					// p.experienceTotal + " " + p.experienceLevel + " " +
					// p.getScore());
					...
					//
					//
					// for (p.experienceTotal += dec; p.experience >= 1.0F;
					// p.experience /= (float)p.xpBarCap())
					// {
					// p.experience = (p.experience - 1.0F) *
					// (float)p.xpBarCap();
					// p.addExperienceLevel(1);
					// }
					//
	...
	public void onUnload(EntityDust e)
	{
		...
		// double tx = x + ((Math.random() > 0.5D) ? 1:-1) +
		// Math.random()*0.4D-0.2D;
		// double tz = z + ((Math.random() > 0.5D) ? 1:-1) +
		// Math.random()*0.4D-0.2D;
		// EntityXPOrb ex = new EntityXPOrb(e.worldObj, tx + 0.5D, y, tz+0.5D,
		// k);
		// ex.motionX = ex.motionY = ex.motionZ = 0;
		// e.worldObj.spawnEntityInWorld(ex);
		// }
		// EntityXPOrb eob = new EntityXPOrb(e.worldObj, e.posX, e.posY +
		// e.yOffset, e.posZ);
		super.onUnload(e);
	}
