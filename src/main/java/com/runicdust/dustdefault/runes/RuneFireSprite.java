 ...
 */
public class RuneFireSprite extends PoweredEvent
{
	public RuneFireSprite()
	{
		super();
	}
	...
	public void onInit(EntityDust e)
	{
		...
		e.setFollow(true);
		e.setColorStarInner(255, 0, 0);
		ItemStack[] sacrifice = new ItemStack[] {
				new ItemStack(Item.ghastTear, 1),
				new ItemStack(Item.fireballCharge, 2) };
		...
		if (e.ticksExisted % 100 == 0 && Math.random() < 0.5)
		{
			...
			for (int x = -rad; x <= rad && !ignited; x++)
			{
				for (int y = -rad; y <= rad && !ignited; y++)
				{
					...
					{
						if (e.worldObj.getBlockId(ex + x, ey + y - 1, ez + z) != 0
								&& e.worldObj
										.getBlockId(ex + x, ey + y, ez + z) == 0
								&& Math.random() < 0.05D)
						{
							e.worldObj.setBlock(ex + x, ey + y, ez + z,
