...
import com.runicdust.dustcore.entity.EntityAIDustFollowBaitRune;
 ...
 */
public class RuneBaiting extends PoweredEvent
{
	public RuneBaiting()
	{
		super();
	...
	public void onTick(EntityDust e)
	{
		...
		for (Entity k : bait)
		{
			// if(k instanceof EntityCreature) System.out.println("ENT " +
			// EntityList.getEntityID(k));
			if (k instanceof EntityCreature
					&& EntityList.getEntityID(k) == e.data[0])
			{
				EntityCreature el = (EntityCreature) k;

				// System.out.println("Found entity " +
				// mod_DustMod.isAIEnabled(el));
				// if (!DustModBouncer.isAIEnabled(el))
				// {
				// el.posY += 1;
				// el.setPathToEntity(null);//e.worldObj.getPathToEntity(el, e,
				// 16F));
				// el.setTarget(e);
				// el.setPathToEntity(e.worldObj.getEntityPathToXYZ(el,
				// e.getX(), e.getY(), e.getZ(), 10F, true, false, false,
				// true));
				// DustModBouncer.updateState(el);
				// el.motionY += 0.015;
				// EntityLookHelper elh = el.getLookHelper();//func_46008_aG();
				// elh.setLookPositionWithEntity(e, 0, 1);//func_46141_a(e, 1,
				// 1);
	...
	public static int getEntity(ItemStack is)
	{
		...
		for (ItemStack i : entdrops.keySet())
		{
			...
			if (i.itemID == is.itemID
					&& i.getItemDamage() == is.getItemDamage())
			{
