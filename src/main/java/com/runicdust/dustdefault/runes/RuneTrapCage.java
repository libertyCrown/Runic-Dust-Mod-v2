 ...
 */
public class RuneTrapCage extends RuneTrap
{
	public RuneTrapCage()
	{
		super();
	...
	{
		e.setRenderStar(true);
		ItemStack[] req = new ItemStack[] { new ItemStack(Item.ingotIron, 6),
	...
	public void trigger(EntityDust e, int dustLevel)
	{
		...
		for (Entity k : trap)
		{
			if (k instanceof EntityLiving)
			{
				...
				el.setPosition((double) x + 0.5D, (double) y + el.yOffset,
						(double) z + 0.5D);
				...
				for (int ix = -1; ix <= 1; ix++)
					for (int iy = 0; iy <= 1; iy++)
						for (int iz = -1; iz <= 1; iz++)
						{
							...
							world.setBlock(x + ix, y + iy, z + iz,
									Block.fenceIron.blockID, 0, 3);
						}
				...
				if (world.getBlockId(x, y - 1, z) == 0)
				{
					for (Integer[] p : e.dustPoints)
					{
						TileEntityDust ted = (TileEntityDust) world
								.getBlockTileEntity(p[0], p[1], p[2]);
