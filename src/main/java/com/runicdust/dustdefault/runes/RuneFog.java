 ...
 */
public class RuneFog extends PoweredEvent
{
	public RuneFog()
	{
		super();
	...
	{
		super.onInit(e);
		ItemStack[] req = new ItemStack[] { new ItemStack(Item.bucketWater, 1),
	...
	public void onTick(EntityDust e)
	{
		...
		if (e.ticksExisted % 5 == 0)
		{
			...
			// int light = e.worldObj.getSavedLightValue(EnumSkyBlock.Block, x,
			// y, z);
			// if (light < 7 && (block == null || !block.isOpaqueCube()))
			// {
			// mod_DustMod.spawnNewFogFX(e.worldObj, x, y, z, 0.85F +
			// (float)Math.random() * 0.2F);
			// }
			// }
			// System.out.println("Alpha" + alpha + " \tPercent" + percent +
			// " \tStage" + stage);
			...
			if (e.ticksExisted % cycle == 0 && !e.worldObj.isRemote)
			{
				...
				EntityPlayer player = e.worldObj.getClosestPlayerToEntity(e,
						radius);
				...
				for (Object o : ents)
				{
					...
					int light = e.worldObj.getSavedLightValue(
					...
					if (i instanceof EntityCreature)
					{
						...
						// Entity target =
						// (Entity)ents2.get((int)(Math.random()*(double)ents2.size()));
						// if(target instanceof EntityLiving && target != ec &&
					...
					if (i instanceof EntityLiving && Math.random() < 0.8D)
					{
						...
						el.setPositionAndRotation(el.posX, el.posY, el.posZ,
								(float) Math.random() * 360F, el.rotationPitch);
