package com.runicdust.dustcore.util;
...
public class VoidTeleManager
{
	...
	public static int[] toWarp(EntityDust e)
	{
		return new int[] {
				e.getX(),
				e.dustPoints.get(0)[1],
				e.getZ(),
				e.data[0],
				e.worldObj.getBlockMetadata(e.getX(),
						e.dustPoints.get(0)[1] - 1, e.getZ()),
				(int) e.rotationYaw, e.worldObj.provider.dimensionId,
				References.warpVer };
	}
		...
		if (propNet != null)
		{
			try
			{
				...
				for (int i = 0; i < clone.size(); i++)
				{
					...
					propNet.setProperty(i + "", "[" + warp[0] + "," + warp[1]
	...
	public static void load(String savePath)
	{
		...
		if (netFS == null || !netFS.exists())
		{
			try
			{
				netFS = new File((new StringBuilder()).append(savePath)
						.append("dustmodvoidnetwork.dat").toString());
			...
			for (Object o : propNet.keySet())
			{
				try
				{
					...
					if (data.length() > 0 && data.charAt(c) == '[')
					{
						...
						int ver;
						// pos =
						// Integer.valueOf(inv.substring(c,inv.indexOf(':',c)));
						// c = inv.indexOf(':',c)+1;
						x = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						y = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						z = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						block = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						meta = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						rot = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						dim = Integer.valueOf(data.substring(c,
								data.indexOf(',', c)));
						c = data.indexOf(',', c) + 1;
						ver = Integer.valueOf(data.substring(c,
								data.indexOf(']', c)));
						c = data.indexOf(']', c);
						voidNetwork.add(new int[] { x, y, z, block, meta, rot,
								dim, ver });
						// System.out.println("Void Network loading: [" + x +
						// "," + y + "," + z + "] " + block + ":" + meta +
						// " dim:" + dim + " ver:" + ver);
					}
				} catch (Exception e)
