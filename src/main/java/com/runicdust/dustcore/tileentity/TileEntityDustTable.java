package com.runicdust.dustcore.tileentity;
...
public class TileEntityDustTable extends TileEntity
{
	...
	public void updateEntity()
	{
		...
		// for (; rotAmt < -(float)Math.PI; rotAmt += ((float)Math.PI * 2F)) { }
		// float f;
		// for (f = rotAmt - rotation; f >= (float)Math.PI; f -= ((float)Math.PI
		// * 2F)) { }
		...
		// bstring = bstring.substring(0,bstring.length() > 4 ?
		// 4:bstring.length());
		// dstring = dstring.substring(0,dstring.length() > 4 ?
		// 4:dstring.length());
		// estring = estring.substring(0,estring.length() > 4 ?
		// 4:estring.length());
		// fstring = fstring.substring(0,fstring.length() > 4 ?
		// 4:fstring.length());
		// hstring = hstring.substring(0,hstring.length() > 4 ?
		// 4:hstring.length());
		// qstring = qstring.substring(0,qstring.length() > 4 ?
		// 4:qstring.length());
		// System.out.println("B " + bstring + " \tD " + dstring + " \tE " +
		// estring + " \tF " + fstring + " \tH " + hstring + " \tQ " + qstring +
		// " " + dir);
	}
