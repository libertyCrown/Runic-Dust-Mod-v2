...
public class EntityBlock extends EntityFallingSand
{
	...
	public EntityBlock(World par1World, double par2, double par4, double par6,
			int par8)
	{
	...
	public void updateSand()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			if (this.onGround)
			{
				...
				if (this.worldObj.getBlockId(var1, var2, var3) != Block.pistonMoving.blockID)
				{
					...
					if (this.worldObj.canPlaceEntityOnSide(this.blockID, var1,
							var2, var3, true, 1, (Entity) null, null)
							&& !BlockSand.canFallBelow(this.worldObj, var1,
									var2 - 1, var3)
							&& this.worldObj.setBlock(var1, var2, var3,
									this.blockID, this.metadata, 3))
					{
						if (Block.blocksList[this.blockID] instanceof BlockSand)
						{
							((BlockSand) Block.blocksList[this.blockID])
									.onFinishFalling(this.worldObj, var1, var2,
											var3, this.metadata);
						}
		...
		double knockback = 2D;
		Vec3 var17 = this.worldObj.getWorldVec3Pool().getVecFromPool(this.posX,
				this.posY, this.posZ);
		...
		if (var5 != null)
		{
			if (var5.attackEntityFrom(DamageSource.inWall, 104))
			{
				if (var5 instanceof EntityLivingBase)
				{
					...
					if (var25 > 0.0F)
					{
						var5.addVelocity(this.motionX * (double) knockback
								* 0.6000000238418579D / (double) var25, 0.1D,
	...
	public boolean doesCollideWithWorld()
	{
		...
		for (int i = -1; i <= 1; i++)
		{
			for (int j = -1; j <= 1; j++)
			{
				for (int k = -1; k <= 1; k++)
				{
					if (Math.abs(i + j) != 1 && Math.abs(i + k) != 0)
						continue;
	...
	public boolean canPlace(int x, int y, int z)
	{
		...
		if (blockID == 0)
			return true;
		if (blockID == this.blockID && metadata == this.metadata)
			return true;
