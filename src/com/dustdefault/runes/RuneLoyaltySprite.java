 ...
 */
public class RuneLoyaltySprite extends PoweredEvent
{
	public RuneLoyaltySprite()
	{
		super();
	}
	...
	public void onInit(EntityDust e)
	{
		...
		e.reanimate = true;
		EntityPlayerMP mp = (EntityPlayerMP) e.worldObj
				.getPlayerEntityByName(e.summonerUN);
		if (mp != null)
		{
	...
	public void onTick(EntityDust e)
	{
		...
		{
			e.posY = e.getY() - 1
					+ MathHelper.sin((float) e.ticksExisted / 16F) * 0.5F;
		...
		{
			EntityPlayer player = e.worldObj
					.getPlayerEntityByName(e.summonerUN);

			// if(true/*!e.worldObj.multiplayerWorld*/) player =
			...
			if (e.toFollow == null)
			{
				...
				for (Entity i : ents)
				{
					if (i instanceof EntityLiving
							&& !(i instanceof EntityPlayer)
							&& EntityList.getEntityID(i) == e.data[0])
					{
			...
			{
				attacking = (((EntityCreature) slave).getEntityToAttack() != null && ((EntityCreature) slave)
						.getEntityToAttack() != player);
			}
			...
			if (!attacking)
				for (Entity i : ents)
				{
					...
					if (i instanceof IMob
							&& (!(i instanceof EntityCreeper) /*
															 * || slave
															 * instanceof
															 * EntityCreeper
															 */))
					{
						// System.out.println("Attacking mob " + (i instanceof
						// EntityCreeper));
						slave.setAttackTarget((EntityCreature) i);
						attacking = true;
						break;
					} else if (i instanceof EntityLiving
							&& /* i instanceof EntityAnimal && */slave instanceof IMob
							&& slave instanceof EntityCreature && !attacking
							&& !(i instanceof EntityCreeper))
					{
			...
			if (slave instanceof EntityCreature)
			{
				...
				if (cSlave.getEntityToAttack() == player
						&& !(slave instanceof EntityAnimal || slave instanceof EntityWolf))
				{
				...
				if (!attacking)
				{
					...
					// cSlave.getDistanceSqToEntity(player));
					if (player.onGround
							&& cSlave.getDistanceSqToEntity(player) > 256D)
					{
						cSlave.setLocationAndAngles(player.posX, player.posY
								+ player.yOffset, player.posZ, 0, 0);
					}
