 ...
 */
public class RuneSpawnerReprog extends DustEvent
{
	public RuneSpawnerReprog()
	{
		super();
	}
	...
	public void onTick(EntityDust e)
	{
		...
		{
			String mob = EntityList.getEntityString(EntityList
					.createEntityByID(e.data[0], e.worldObj));
			int[] fin = new int[3];
			...
			{
				TileEntityMobSpawner tems = ((TileEntityMobSpawner) e.worldObj
						.getBlockTileEntity(fin[0], fin[1], fin[2]));
				// TODO-see if this is fixed
				tems.getSpawnerLogic().setMobID(mob);
				tems.validate();

				// e.worldObj.setBlockWithNotify(fin[0], fin[1], fin[2],0);
				// e.worldObj.setBlockWithNotify(fin[0], fin[1], fin[2],
				...
				PacketDispatcher.sendPacketToAllAround(fin[0], fin[1], fin[2],
						64, e.worldObj.getWorldInfo().getVanillaDimension(),
						tems.getDescriptionPacket());
