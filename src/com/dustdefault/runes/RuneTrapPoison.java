 ...
 */
public class RuneTrapPoison extends RuneTrap
{
	@Override
	...
	public void trigger(EntityDust e, int dustLevel)
	{
		...
		for (Entity k : kill)
		{
			...
			{
				((EntityLiving) k).addPotionEffect(new PotionEffect(
						Potion.poison.id,
						(poisondambase + ((int) Math.floor(Math.random()
								* (double) poisondamrand))) * 20, 2));
			}
		}
