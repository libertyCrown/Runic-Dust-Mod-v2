						...
						-0.10000000149011612D), Material.fire))
		{
					...
					&& buttons[0] && !wasClickDown(item))
			{
				if (!wearer.worldObj.isRemote)
				{
					...
					EntitySmallFireball fire = new EntitySmallFireball(
							wearer.worldObj, wearer, look.xCoord, look.yCoord,
							look.zCoord);
					...
					fire.setLocationAndAngles(wearer.posX, wearer.posY,
							wearer.posZ, wearer.rotationYaw,
							wearer.rotationPitch);
					fire.setPosition(fire.posX + ox,
							fire.posY + wearer.getEyeHeight() - 0.2, fire.posZ
									+ oz);
					fire.yOffset = 0.0F;
					fire.motionX = fire.motionY = fire.motionZ = 0.0D;
					double var9 = (double) MathHelper.sqrt_double(par3 * par3
							+ par5 * par5 + par7 * par7);
