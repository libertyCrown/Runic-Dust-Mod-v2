...
public class ErfBendInscription extends InscriptionEvent {
	...
	public boolean callSacrifice(DustEvent rune, EntityDust e, ItemStack item) {
		ItemStack[] req = rune.sacrifice(e, new ItemStack[] {new ItemStack(Block.blockIron, 8, -1)});
			...
			for(int j = -r; j <= r; j++){
				for(int k = 0; k > -launch; k--){
					...
					if(!isSolid(toBid,toMeta)/* && Block.blocksList[bid].isOpaqueCube()*/){
						world.setBlock(tx+i, ty+k+launch, tz+j, bid,meta,3);
						world.setBlock(tx+i, ty+k, tz+j, getFillerMaterial(bid,meta),0,3);
					}
				}
			}
		}
				...
				for(int i = 1; i >= -1; i--){
					for(int j = 1; j >= -1; j--){
						for(int k = 1; k >= -1; k--){
							...
							world.setBlock(bPos[0]+i, bPos[1]+j, bPos[2]+k,getCompressedMaterial(bid,meta),0,3);
						}
					}
				}
			}
					...
					for(int i = 1+ax; i >= -1+ax; i--){
						for(int j = 1+ay; j >= -1+ay; j--){
							for(int k = 1+az; k >= -1+az; k--){
								...
								world.setBlock(tx+i+mx, ty+j+my, tz+k+mz,bid,meta,3);
...
//					world.setBlockAndMetadataWithNotify(tx+i, ty+j+h, tz+k,bid,meta);
					world.setBlock(bPos[0]+i, bPos[1]+j, bPos[2]+k,0,0,3);
				}
			}
		}
		for(int i = 1; i >= -1; i--){
