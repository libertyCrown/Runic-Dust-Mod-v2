...
public class LaunchTestRune extends DustEvent
{
	...
	public void launchToward(Entity e, double xf, double yf, double zf)
	{
		...
		d = Math.sqrt((xi - xf) * (xi - xf) + (yi - yf) * (yi - yf) + (zi - zf)
				* (zi - zf));
		...
		e.addVelocity(Math.sin(rotTheta) * horizVel, Math.sin(theta) * vi,
				Math.cos(rotTheta) * horizVel);

		if (e instanceof EntityPlayer)
		{
			System.out.println("You wretched whore");
			EntityPlayer ent = (EntityPlayer) e;
			ent.addMovementStat(Math.sin(rotTheta) * horizVel, Math.sin(theta)
					* vi, Math.cos(rotTheta) * horizVel);
			;
		}
	}
}
