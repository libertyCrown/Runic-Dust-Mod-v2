...
{
	private ResourceLocation beam = new ResourceLocation(References.path
			+ "/beam.png");
	...
	public void renderStar(EntityDust g, float f, float ri, float gi, float bi,
			float ro, float go, float bo, float scale)
	{
		...
		for (int i = 0; (float) i < ((f1 + f1 * f1) / 2F) * 90F + 30F; i++)
		{
			...
			GL11.glRotatef(random.nextFloat() * 360F, 1.0F, 0.0F, 0.0F);
			GL11.glRotatef(random.nextFloat() * 360F, 0.0F, 1.0F, 0.0F);
			GL11.glRotatef(random.nextFloat() * 360F + f1 * 90F, 0.0F, 0.0F,
					1.0F);
			...
			tessellator.addVertex(-0.86599999999999999D * (double) f4, f3,
					-0.5F * f4);
			tessellator.addVertex(0.86599999999999999D * (double) f4, f3, -0.5F
					* f4);
	...
	public void renderBeam1(EntityDust e, double x, double y, double z,
			double i, double j, double k, float f)
	{
		GL11.glPushAttrib(GL11.GL_LIGHTING_BIT | GL11.GL_ENABLE_BIT
				| GL11.GL_COLOR_BUFFER_BIT);
		...
		GL11.glRotatef(((float) (-Math.atan2(f6, f4)) * 180F) / (float) Math.PI
				- 90F, 0.0F, 1.0F, 0.0F);
		GL11.glRotatef(((float) (-Math.atan2(f7, f5)) * 180F) / (float) Math.PI
				- 90F, 1.0F, 0.0F, 0.0F);
		Tessellator tessellator = Tessellator.instance;
		RenderHelper.disableStandardItemLighting();
		GL11.glDisable(GL11.GL_CULL_FACE);
		...
		float f10 = MathHelper.sqrt_float(f4 * f4 + f5 * f5 + f6 * f6) / 32F
				- ((float) (ticks)) * 0.01F;
		...
		{
			float f11 = MathHelper
					.sin(((float) (jt % it) * (float) Math.PI * 2.0F)
							/ (float) it) * 0.75F;
			float f12 = MathHelper
					.cos(((float) (jt % it) * (float) Math.PI * 2.0F)
							/ (float) it) * 0.75F;
	...
	public void renderBeam2(EntityDust e, double x, double y, double z,
			double i, double j, double k, float f)
	{
		float var9 = 1f;
		...
		if (var9 > 0.0F)
		{
			...
			float var12 = -var11 * 0.2F
					- (float) MathHelper.floor_float(-var11 * 0.1F);
			byte var13 = 1;
			double var14 = (double) var11 * 0.025D
					* (1.0D - (double) (var13 & 1) * 2.5D);
			var10.startDrawingQuads();
			GL11.glEnable(GL11.GL_BLEND);
			GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
			GL11.glDepthMask(false);
			RenderHelper.disableStandardItemLighting();
			var10.setColorRGBA(191 + e.rb / 4, 191 + e.gb / 4, 191 + e.bb / 4,
					128);
	...
	public void renderFlames(EntityDust e, float par2)
	{
		...
		boolean renderFlamesRut = (References.Enable_Render_Flames_On_Ruts
				&& e.renderFlamesRut && e.rutPoints != null);
		...
		if (renderFlamesRut && e.flameRenderHelperRut == null
				&& e.rutPoints != null)
		{
		...
		if (renderFlamesDust)
			for (int iter = 0; iter < e.dustPoints.size(); iter++)
			{
				...
				GL11.glPushMatrix();
				GL11.glTranslatef((float) p[0] - i, (float) p[1] - j,
						(float) p[2] - k);
		...
		if (renderFlamesRut)
			for (int iter = 0; iter < e.rutPoints.size(); iter++)
			{
				...
				GL11.glPushMatrix();
				GL11.glTranslatef((float) p[0] - i, (float) p[1] - j + 1,
						(float) p[2] - k);
				renderBlockFire(e.worldObj, i, j + 1, k, e.rf, e.gf, e.bf,
						sides);
				GL11.glPopMatrix();
			}
			...
			int b, Integer[] sides)
	{
		...
		tessellator.setBrightness(block.getMixedBrightnessForBlock(world, i, j,
				k));
		float f4 = 1.0F;
		float f5 = 1.0F;
		...
		tessellator.setColorOpaque_F((float) (r / 255), (float) (g / 255),
				(float) (b / 255));
			...
			float f1)
	{
		...
		if (dust.renderBeam)
		{
			...
			{
				case 0:
					renderBeam1(dust, d, d1/* +EntityDust.yOffset */, d2, d,
							256, d2, f);
					break;
				case 1:
					renderBeam2(dust, d, d1/* +EntityDust.yOffset */, d2, d,
							256, d2, f);
					break;
				default:
					renderBeam1(dust, d, d1/* +EntityDust.yOffset */, d2, d,
