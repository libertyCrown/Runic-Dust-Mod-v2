...
public class GuiInscription extends GuiContainer
{
			...
			int var3)
	{
		...
		if (inscription == null
				|| inscription.itemID != DustContent.inscription.itemID)
		{
	...
	protected void drawGuiContainerForegroundLayer(int a, int b)
	{
		...
		if (info.getHasStack() && insc.canEdit())
		{
			...
			this.drawTexturedModalRect(highlightSlot.xDisplayPosition - 2,
					highlightSlot.yDisplayPosition - 2, 0, ySize + 6, 20, 20);
		}
		...
		for (int x = 0; x < 16; x++)
		{
			for (int y = 0; y < 16; y++)
			{
				...
				{
					drawCell(x, y,
							DustItemManager.getPrimaryColor(getDust(x, y)));
				}
			}
		}
	...
	public void sendData()
	{
		...
		if (cont)
		{
			FMLClientHandler.instance().sendPacket(
					PacketHandler.getSetInscriptionPacket(insc.inv));
		}
	}
	...
	protected void mouseClicked(int x, int y, int b)
	{
				...
				&& slot.getStack().itemID == DustContent.ink.itemID)
		{
			...
			insc.setInventorySlotContents(0, new ItemStack(
					DustContent.ink.itemID, id, slot.slotNumber - 1));
		}
		if (isOnMap(x, y))
		{
	...
	protected void mouseMovedOrUp(int x, int y, int b)
	{
		...
		if (isDown)
		{
			...
			if (button == 0
					&& this.inventorySlots.getSlot(0).getStack() != null)
			{
				int slot = this.inventorySlots.getSlot(0).getStack()
						.getItemDamage();
				int id = this.inventorySlots.getSlot(0).getStack().stackSize;
				ItemStack stack = this.playerInv.getStackInSlot(slot);
				if (getDust(x, y) != id
						&& ItemInk.reduce(this.player, stack, 1))
				{
					setDust(x, y, id);
					FMLClientHandler.instance().sendPacket(
	...
	public boolean isOnMap(int x, int y)
	{
		...
		x -= this.guiLeft;
		y -= this.guiTop;
		return x > 42 - tol && x < 42 + 16 * 6 + tol && y > 19 - tol
				&& y < 19 + 16 * 6 + tol;
	}
	...
	{
		changed = true;
		this.insc.setInventorySlotContents(x * 16 + y + 10, new ItemStack(
				DustContent.inscription.itemID, 1, dust));
		if (newInscription)
		{
			this.sendData();
	...
	{
		return this.getSomething(par1Slot.xDisplayPosition,
				par1Slot.yDisplayPosition, 16, 16, x, y);
	}

	// it has an undefined name, and I don't know what it does, so to avoid
	// getting broken in later versions, I do this
	protected boolean getSomething(int par1, int par2, int par3, int par4,
			int x, int y)
	{
		...
		x -= var7;
		y -= var8;
		return x >= par1 - 1 && x < par1 + par3 + 1 && y >= par2 - 1
				&& y < par2 + par4 + 1;
	}
