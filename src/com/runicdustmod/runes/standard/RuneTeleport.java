 ...
 */
public class RuneTeleport extends PoweredEvent
{
	...
	public void onInit(EntityDust e)
	{
		...
		for (Integer[] i : e.dustPoints)
		{
			...
			if (te != null && te instanceof TileEntityDust)
			{
				...
				if (gamt == 0 && bamt == 0)
				{
					...
					RunicDustMod.log("Warp ID set to "
                            + e.data[0]
                            + " "
		...
		for (int x = -1; x <= 1 && fnd != null; x++)
		{
			for (int z = -1; z <= 1; z++)
			{
				if (x == 0 || z == 0)
				{
					if (RunicDustMod.isDust(world.getBlockId(fnd[0] + x, fnd[1],
                            fnd[2] + z)))
					{
		...
		// System.out.println("Derp set " + e.data[0] + " " +
		// Block.blocksList[e.data[0]].getBlockName() + " " + e.rot);
		e.posY += 1.5D;
		e.setRenderStar(true);
																 ...
																 */)
				{
					...
					for (int temp = index + 1; temp != index && index != -1; temp++)
					{
						...
						System.out.println("Found warp: " + warp[3] + ":"
								+ warp[4] + " " + iwarp[3] + ":" + iwarp[4]
								+ " dim:" + iwarp[6] + " ver:" + iwarp[7]);
						...
						// if(ed != null && ed.data == e.data[0] &&
						// !ed.equals(e) && ed != e && ed.ram == 1){
						if (warp[3] == iwarp[3] && warp[4] == iwarp[4])
						{
							...
							{
								addFuel(e, -1600);
								((EntityLivingBase) i).setPositionAndRotation(
										iwarp[0] + 0.5D, iwarp[1] + 0.6D,
										iwarp[2] + 0.5D, e.rotationYaw,
										i.rotationPitch);
								((EntityLivingBase) i).setPositionAndUpdate(
										iwarp[0] + 0.5D, iwarp[1] + 0.6D,
										iwarp[2] + 0.5D);
								((EntityLivingBase) i).attackEntityFrom(
							...
							i.setPositionAndRotation(iwarp[0] + 0.5D,
									iwarp[1] + 0.6D, iwarp[2] + 0.5D, iwarp[5],
									i.rotationPitch);
							e.ram = 100;
