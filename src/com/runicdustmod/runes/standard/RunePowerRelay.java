 ...
 */
public class RunePowerRelay extends PoweredEvent
{
	public static int distance = 32;

	public RunePowerRelay()
	{
		super();
	...
	public void disperseFuel(EntityDust e)
	{
		...
		for (EntityDust i : arr)
		{
			...
			if (hasEnough)
			{
				...
				// System.out.println("Giving fuel " + i.getFuel() + " " + prev
				// + " " + want + " " + e.getFuel());
			} else
			{
	...
	public static List<EntityDust> findDustEntities(EntityDust e)
	{
		...
		int z = e.getZ();
		int radius = 32;
		List<EntityDust> rtn = e.worldObj.getEntitiesWithinAABB(
				e.getClass(),
	...
	public void drawConnections(EntityDust e)
	{
		...
		for (EntityDust i : arr)
		{
			drawConnection(e.worldObj, new double[] { e.posX - 0.5, e.posY,
					e.posZ - 0.5 }, new double[] { i.posX - 1.5, i.posY,
					i.posZ - 0.5 });
		}
	}
	...
	public void drawConnection(World world, double[] loc1, double[] loc2)
	{
		...
		d = (loc1[0] - loc2[0]) * (loc1[0] - loc2[0]) + (loc1[1] - loc2[1])
				* (loc1[1] - loc2[1]) + (loc1[2] - loc2[2])
				* (loc1[2] - loc2[2]);
		...
		int iter = 0;
		double[] locations = new double[((int) Math
				.abs((loc2[0] * 2 - loc1[0]) / 0.5) + 1) * 3 + 1];
		...
		for (x = minx; x < maxx; x += 0.5)
		{
			...
			y = -(x * x) / 4 + h;
			y = -Math.pow((x - ((loc2[0] - loc1[0]) / 2)), 2) / (d / h / 3.9)
					+ c + h;
		...
		RunicDustMod.spawnParticles(world, "reddust", locations, 0, 1, 0, 2, 0.01,
                0.01, 0.01);
	}
}
