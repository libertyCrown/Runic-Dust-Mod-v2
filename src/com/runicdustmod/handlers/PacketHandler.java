...
import com.runicdustmod.api.DustItemManager.DustColor;
...
public class PacketHandler implements IPacketHandler, IConnectionHandler
{
	...
	public static Packet getParticlePacket(String type, short formation,
			double[] loc, double velx, double vely, double velz, int amt,
	...
	public static Packet getRenderBrokenToolPacket(EntityLivingBase wearer,
			ItemStack tool)
	{
		ByteArrayOutputStream bos = new ByteArrayOutputStream(140);
		DataOutputStream dos = new DataOutputStream(bos);
	...
	private static void onTEDPacket(byte[] data, Player player)
	{
		...
		int r, g, b;
		TileEntityDust ted;
		World world = RunicDustMod.proxy.getClientWorld();
		...
		try
		{
			...
			for (int y = 0; y < h; y++)
			{
				for (int x = 0; x < w; x++)
				{
					for (int z = 0; z < l; z++)
					{
						design[y][x][z] = dis.readInt();// dos.writeInt(shape.getDataAt(x,
														// y, z));
					}
				}
			}
			...
			DustShape shape = new DustShape(w, l, name, solid, ox, oy, cx, cy,
					pageNumber, id);
			...
			Player player)
	{
		...
		try
		{
			...
			for (int x = 0; x < w; x++)
			{
				for (int y = 0; y < h; y++)
				{
					design[x][y] = dis.readInt();// dos.writeInt(shape.getDataAt(x,
													// y, z));
				}
			}

			InscriptionEvent event = new InscriptionEvent(design, idname, name,
					id);
	...
	private static void onDustDeclarationPacket(byte[] data, Player player)
	{
		...
		try
		{
			...
			DustItemManager.registerRemoteDust(value, name, idName,
					primaryColor, secondaryColor, floorColor);
		} catch (IOException e)
		{
			return;
		}
	...
	private static void onMousePacket(byte[] data, Player player)
	{
		...
		try
		{
			...
			RunicDustMod.keyHandler.setKey(player, keyID, pressed);
	...
	private static void onSetVelocityPacket(byte[] data, Player player)
	{
		...
		try
		{
			...
			EntityLivingBase ent = (EntityLivingBase) ((EntityPlayer) player).worldObj
					.getEntityByID(entID);

			if (ent != null)
			{
				ent.setVelocity(mx, my, mz);
				ent.addVelocity(-ent.motionX + mx, -ent.motionY + my,
						-ent.motionZ + mz);
	...
	private static void onRenderBrokenToolPacket(byte[] data, Player player)
	{
		...
		try
		{
			...
			EntityPlayer ent = (EntityPlayer) ((EntityPlayer) player).worldObj
					.getEntityByID(entID);
			...
			INetworkManager manager)
	{
		...
		RunicDustMod.keyHandler.checkPlayer(player);
		...
		for (DustShape shape : DustManager.shapes)
		{
			...
			if (e.canPlayerKnowRune(((EntityPlayer) player).username)
					&& !e.secret)
		...
		{
			if (evt.canPlayerKnowInscription((EntityPlayer) player)
					&& !evt.secret)
	...
	{
		RunicDustMod.log(Level.FINER, "Resetting due to opened connection.1");
	...
	{
		RunicDustMod.log(Level.FINER, "Resetting due to opened connection.2");
		DustManager.resetMultiplayerRunes();
		DustItemManager.reset();
		InscriptionManager.resetRemoteInscriptions();
	}
