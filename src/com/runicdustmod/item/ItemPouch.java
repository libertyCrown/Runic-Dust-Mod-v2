...
public class ItemPouch extends Item
{
	...
	@Override
	public boolean onItemUse(ItemStack item, EntityPlayer p, World world,
			int i, int j, int k, int face, float x, float y, float z)
	{
		...
		{
			if (world.canPlaceEntityOnSide(this.blockID, i, j, k, false, face,
					(Entity) null, item))
			{
				Block var12 = Block.blocksList[this.blockID];
				int var13 = var12.onBlockPlaced(world, i, j, k, face, x, y, z,
						0);
				...
				if (world.setBlock(i, j, k, this.blockID, 0, 3))
				{
					...
					{
						Block.blocksList[this.blockID].onBlockPlacedBy(world,
								i, j, k, p, item);
						Block.blocksList[this.blockID].onPostBlockPlaced(world,
								i, j, k, var13);
					}
					DustContent.dust.onBlockActivated(world, i, j, k, p, face,
							x, y, z);

					world.playSoundEffect((double) ((float) i + 0.5F),
							(double) ((float) j + 0.5F),
							(double) ((float) k + 0.5F),
							var12.stepSound.getStepSound(),
							(var12.stepSound.getVolume() + 1.0F) / 6.0F,
							var12.stepSound.getPitch() * 0.99F);
	...
	public String getUnlocalizedName(ItemStack itemstack)
	{
		...
		if (id != null)
			return "pouch." + DustItemManager.idsRemote[dust];
		// System.out.println("Dammit " + dust + " " +
	...
	public int getColorFromItemStack(ItemStack stack, int pass)
	{
		...
		return pass == 1 ? DustItemManager.getPrimaryColor(meta)
				: DustItemManager.getSecondaryColor(meta);
	}
	...
	public void addInformation(ItemStack item, List info)
	{
		if (item.hasTagCompound())
		{
			...
			// if (author != null)
			// {
			info.add("\u00a77 Holding " + amt + " piles.");// String.format(StatCollector.translateToLocalFormatted("book.byAuthor",
															// new Object[]
															// {author.data}),
															// new Object[0]));
			// }
	...
	public static int getDustAmount(ItemStack pouch)
	{
		...
		if (pouch.itemID != DustContent.pouch.itemID)
			return -1;
	...
	{
		this.bagIcon = iconRegister.registerIcon(References.spritePath
				+ "dustPouch_back");
		this.mainIcon = iconRegister.registerIcon(References.spritePath
				+ "dustPouch_main");
		this.subIcon = iconRegister.registerIcon(References.spritePath
				+ "dustPouch_sub");
	}
}
