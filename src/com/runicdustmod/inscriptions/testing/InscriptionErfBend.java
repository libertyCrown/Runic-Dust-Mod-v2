...
public class InscriptionErfBend extends InscriptionEvent
{
			...
			boolean[] buttons)
	{
				...
				&& wearer.isSneaking() && wearer.onGround)
		{
			try
			{
				...
				// Pillar & sinkhole
				if (!isAlreadyLaunching(item) && target != null
						&& (buttons[0] || buttons[1]))
				{
					...
					if ((world.getBlockId(x, y, z) == 0 && world.getBlockId(x,
							y - 1, z) == 0) || target.isInWater())
						return;
					if (buttons[0]
							&& checkForEarth(target, r + 1, launch) < 0.80)
						return;
				...
				{
					if (!buttons[1] && this.mouseWasDownR(item))
						blockClick(wearer, item);
					else if (buttons[2])
						middleClickBlock(wearer, item);
				}
		...
		} else if (((EntityPlayer) wearer).getCurrentEquippedItem() == null
				&& buttons[0])
		{
			fire(wearer, item);
		}
		...
		if (isAlreadyLaunching(item))
		{
			...
			if (vec.lengthVector() > eRad)
			{
				...
				for (int i = 5; i >= -5; i--)
				{
					...
					if (isSolid(bid, meta) && !isSolid(bidA, metaA))
					{
						...
						RunicDustMod.spawnParticles(wearer.worldObj, "tilecrack_"
			...
			} else
			{
				...
				int launch = 6;
				if (button == 0)
					launch(world, wearer, tx, ty, tz, r, launch, eRad);
				else
					sink(world, wearer, tx, ty, tz, r, launch, eRad);
			}
		}
	...
	public void launch(World world, Entity wearer, int tx, int ty, int tz,
			int r, int launch, int eRad)
	{
		List<Entity> ents = world.getEntitiesWithinAABBExcludingEntity(
				wearer,
				AxisAlignedBB.getBoundingBox(tx, ty, tz, tx + 1.0D, ty + 1.0D,
						tz + 1.0D).expand(eRad, eRad, eRad));
		for (Entity ent : ents)
		{
			ent.setLocationAndAngles(ent.posX, ent.posY + launch, ent.posZ,
					ent.rotationYaw, ent.rotationPitch);
		...
		for (int i = -r; i <= r; i++)
		{
			for (int j = -r; j <= r; j++)
			{
				for (int k = 0; k > -launch; k--)
				{
					...
					int toBid = world.getBlockId(tx + i, ty + k + launch, tz
							+ j);
					int toMeta = world.getBlockMetadata(tx + i,
							ty + k + launch, tz + j);
					if (bid == 0 && k != 0)
						bid = Block.cobblestone.blockID;
					if (!isSolid(toBid, toMeta)/*
												 * && Block.blocksList[bid].
												 * isOpaqueCube()
												 */)
					{
						world.setBlock(tx + i, ty + k + launch, tz + j, bid,
								meta, 3);
						world.setBlock(tx + i, ty + k, tz + j,
								getFillerMaterial(bid, meta), 0, 3);
					}
				}
			}
		}
	...
	public void sink(World world, Entity wearer, int tx, int ty, int tz, int r,
			int launch, int eRad)
	{
		List<Entity> ents = world.getEntitiesWithinAABBExcludingEntity(
				wearer,
			...
			for (int j = -r; j <= r; j++)
			{
				for (int k = 0; k > -launch; k--)
				{
					...
					int toMeta = world.getBlockMetadata(tx + i,
							ty + k + launch, tz + j);
					if (bid == 0)
						continue;// bid = Block.cobblestone.blockID;
					if (!isSolid(toBid, toMeta)/*
												 * && Block.blocksList[bid].
												 * isOpaqueCube()
												 */)
					{
						world.setBlock(tx + i, ty + k - launch, tz + j, bid,
								meta, 3);
						world.setBlock(tx + i, ty + k, tz + j, 0, 0, 3);
					}
				}
			}
		}
	...
	public void targetEntity(ItemStack item, Entity player, Entity target,
			int button)
	{
		NBTTagCompound tag = item.getTagCompound();
	...
	public boolean isSolid(int bid, int meta)
	{
		...
		if (m == Material.vine || m == Material.ice || m == Material.wood
				|| m == Material.web)
			return false;
		if (m == Material.snow)
		{
	...
	{
		// final int sand=Block.sand.blockID,
		// stone=Block.stone.blockID,dirt=Block.dirt.blockID,gravel=Block.gravel.blockID,netherbrick=Block.netherBrick.blockID;
	...
	public boolean isCompressedMaterial(int blockID)
	{
		int[] bids = new int[] { Block.sandStone.blockID,
				Block.cobblestone.blockID, Block.cobblestoneMossy.blockID,
	...
	public static synchronized Entity getLaunchEnt(EntityPlayer ep)
	{
		try
		{
			...
			Vec3 var8 = var6.addVector(var7.xCoord * var11,
					var7.yCoord * var11, var7.zCoord * var11);
			List var10 = ep.worldObj.getEntitiesWithinAABBExcludingEntity(
					ep,
					ep.boundingBox.addCoord(var7.xCoord * var11,
							var7.yCoord * var11, var7.zCoord * var11).expand(
			...
			while (var13.hasNext())
			{
				...
				if (var14.canBeCollidedWith())
				{
					...
					AxisAlignedBB var16 = var14.boundingBox.expand(
							(double) var15, (double) var15, (double) var15);
					MovingObjectPosition var17 = var16.calculateIntercept(var6,
							var8);
			...
			if (isCloseToMovingBlock(item, tx, ty, tz))
			{
				...
				for (int i = 1; i >= -1; i--)
				{
					for (int j = 1; j >= -1; j--)
					{
						for (int k = 1; k >= -1; k--)
						{
							int bid = world.getBlockId(bPos[0] + i,
									bPos[1] + j, bPos[2] + k);
							int meta = world.getBlockMetadata(bPos[0] + i,
									bPos[1] + j, bPos[2] + k);
							world.setBlock(bPos[0] + i, bPos[1] + j, bPos[2]
									+ k, getCompressedMaterial(bid, meta), 0, 3);
						}
					}
				}
			}
			...
			for (int j = 1; j >= -1; j--)
			{
				for (int k = 1; k >= -1; k--)
				{
					...
					if (isCompressedMaterial(bid))
					{
						...
						EntityArrow arrow = new EntityArrow(world, x + 0.5,
								y + 0.5, z + 0.5);
						// EntityFallingSand block = new
						// EntityFallingSand(world,(double)x+0.5,(double)y+0.5,(double)z+0.5,bid);
						Vec3 vel;
						if (target == null)
						{
						...
						{
							vel = Vec3.createVectorHelper(target.posX
									- (x + 0.5), target.posY - (y + 0.5) + 1.5,
									target.posZ - (z + 0.5));
						}
						...
						EntityBlock eb = new EntityBlock(world,
				...
				if (dist < tol * tol)
				{
					...
					for (int i = 1 + ax; i >= -1 + ax; i--)
					{
						for (int j = 1 + ay; j >= -1 + ay; j--)
						{
							...
							{
								int bid = world.getBlockId(tx + i, ty + j, tz
										+ k);
								int meta = world.getBlockMetadata(tx + i, ty
										+ j, tz + k);

								if (j == -1 + ay
										&& Block.blocksList[bid] instanceof BlockSand)
								{
									bid = Block.sandStone.blockID;
									meta = 0;
								}

								world.setBlock(tx + i + mx, ty + j + my, tz + k
										+ mz, bid, meta, 3);
								world.setBlock(tx + i, ty + j, tz + k, 0, 0, 3);
							}
						}
					}
					setMovingBlock(item, tx + ax + mx, ty + ay + my, tz + az
							+ mz);
				}
			}
		}
	...
	public void moveBlock(World world, ItemStack item, int clickx, int clicky,
			int clickz, int face)
	{
		...
		for (int i = 1; i >= -1; i--)
		{
			for (int j = 1; j >= -1; j--)
			{
				for (int k = 1; k >= -1; k--)
				{
					int bid = world.getBlockId(bPos[0] + i, bPos[1] + j,
							bPos[2] + k);
					int meta = world.getBlockMetadata(bPos[0] + i, bPos[1] + j,
							bPos[2] + k);
					blocks[i + 1][j + 1][k + 1][0] = bid;
					blocks[i + 1][j + 1][k + 1][1] = meta;
					// world.setBlockAndMetadataWithNotify(tx+i, ty+j+h,
					// tz+k,bid,meta);
					world.setBlock(bPos[0] + i, bPos[1] + j, bPos[2] + k, 0, 0,
							3);
				}
			}
		}
		for (int i = 1; i >= -1; i--)
		{
			for (int j = 1; j >= -1; j--)
			{
				for (int k = 1; k >= -1; k--)
				{
					...
					world.setBlock(bPos[0] + i + ax, bPos[1] + j + ay, bPos[2]
							+ k + az, bid, meta, 3);
				}
			}
		}
