 ...
 * <code>http://xml.org/sax/features/namespace-prefixes</code> feature is set to
 * <var>true</var> (it is <var>false</var> by default). Because SAX2 conforms to
 * the original "Namespaces in XML" recommendation, it normally does not give
 * namespace declaration attributes a namespace URI.
 * </p>
 * 
 * <p>
 * Some SAX2 parsers may support using an optional feature flag (
 * <code>http://xml.org/sax/features/xmlns-uris</code>) to request that those
 * attributes be given URIs, conforming to a later backwards-incompatible
 * revision of that recommendation. (The attribute's "local name" will be the
 * prefix, or "xmlns" when defining a default element namespace.) For
 * portability, handler code should always resolve that conflict, rather than
 * requiring parsers that can change the setting of that feature flag.
 * </p>
 * 
 * <p>
 * If the namespace-prefixes feature (see above) is <var>false</var>, access by
 ...
 * {@link org.xml.sax.AttributeList AttributeList} interface, which does not
 * contain Namespace support. In addition to Namespace support, it adds the
 * <var>getIndex</var> methods (below).
 * </p>
 * 
 * <p>
