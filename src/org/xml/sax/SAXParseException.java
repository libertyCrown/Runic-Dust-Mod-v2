 ...
 * XML document, as if it came from a {@link Locator} object. Note that although
 * the application will receive a SAXParseException as the argument to the
 * handlers in the {@link org.xml.sax.ErrorHandler ErrorHandler} interface, the
...
public class SAXParseException extends SAXException
{
	 ...
	 * own exception from within a {@link org.xml.sax.ContentHandler
	 ...
	 * This constructor is especially useful when an application is creating its
	 * own exception from within a {@link org.xml.sax.ContentHandler
	 * ContentHandler} callback, and needs to wrap an existing exception that is
	 ...
	 * {@link Locator}. For example, if the system identifier is a URL
	 * (including relative filename), the caller must resolve it fully before
	 ...
	 * exception that is not a subclass of {@link org.xml.sax.SAXException
	 * SAXException}.
	 * </p>
	 * 
	 * <p>
	 * All parameters except the message and exception are as if they were
	 * provided by a {@link Locator}. For example, if the system identifier is a
	 * URL (including relative filename), the caller must resolve it fully
