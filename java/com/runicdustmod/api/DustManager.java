...
public class DustManager
{
	...
	{
		ArrayList<DustShape> s = (RunicDustMod.proxy.isClient()) ? shapesRemote
				: shapes;
	...
	{
		RunicDustMod.log(Level.FINE, "Reseting remote runes.");
		namesRemote = new ArrayList<String>();
		shapesRemote = new ArrayList<DustShape>();
		RunicDustMod.proxy.resetPlayerTomePage();
	}
			...
			DustEvent eventInstance)
	{
		...
		shapes.add(shape);
		RunicDustMod.proxy.checkRunePage(shape);
		if (eventInstance != null && eventInstance instanceof PoweredEvent)
		{
			shape.isPower = true;
		}

		RunicDustMod.log(Level.FINER, "Registering rune: " + shape.name);

		if (config == null)
		{
			config = new Configuration(RunicDustMod.runeConfig);
			config.load();
		...
		if (!eventInstance.secret)
		{
			...
			{
				RunicDustMod.log(Level.FINE, "Rune permission for "
	...
	public static void registerRemoteDustShape(DustShape shape)
	{
		...
		RunicDustMod.proxy.checkRunePage(shape);
		RunicDustMod.log(Level.FINER, "Registering remote rune: " + shape.name);
		LanguageRegistry.instance().addStringLocalization(
			...
			int[][] map, List<Integer[]> points, String username)
	{
		...
		for (int[] a : trim)
		{
			for (int b : a)
			{
				if (b == -2)
				{
					...
					RunicDustMod.log(Level.FINER, "Left variable dust in rune.");
					return;
				}
			}
		}
		...
		if (found != null)
		{
			RunicDustMod.log(Level.FINER, "Found rune: " + found.name);
		...
		} else
		{
			...
			RunicDustMod.log(Level.FINER, "No rune found.");
		}
	}
