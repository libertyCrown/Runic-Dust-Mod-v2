 ...
 */
public class RuneSarlacc extends PoweredEvent
{
	public RuneSarlacc()
	{
		super();
	...
	public void onTick(EntityDust e)
	{
		...
		for (Entity et : omnom)
		{
			...
			if (et instanceof EntityItem)
			{
				et.setDead();
				this.addFuel(e,
						((EntityItem) et).getEntityItem().stackSize * 10);
			}
				...
				for (int mul = 0; mul < 2; mul++)
					for (int i = exp; i > 0;)
					{
						...
						double tz = z + ((Math.random() > 0.5D) ? 1 : -1)
								+ Math.random() * 0.4D - 0.2D;
						EntityXPOrb ex = new EntityXPOrb(e.worldObj, tx + 0.5D,
								y, tz + 0.5D, k);
			...
			} else if (et == player)
			{
				...
				{
					((EntityPlayer) player).attackEntityFrom(
