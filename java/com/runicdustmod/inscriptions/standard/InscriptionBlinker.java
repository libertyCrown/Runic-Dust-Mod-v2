...
public class InscriptionBlinker extends InscriptionEvent
{
	...
	public InscriptionBlinker(int[][] design, String idName, String properName,
			int id)
	{
	...
	public boolean callSacrifice(DustEvent rune, EntityDust e, ItemStack item)
	{
		...
		ItemStack blinkI = new ItemStack(DustContent.wornInscription.itemID, 1,
				0);
				...
				&& wearer.isSneaking())
		{
			...
			if (target != null)
			{
				double distT = (target[0] - x) * (target[0] - x)
						...
						* (target[2] - z);
				// double distL = (testLoc[0] - x)*(testLoc[0]-x) +
				// (testLoc[1]-y)*(testLoc[1] - y) +
				// (testLoc[2]-z)*(testLoc[2]-z);
				if (distT < dist * dist)
				{
			...
			RunicDustMod.spawnParticles(wearer.worldObj, "reddust", testLoc,
                    canTele ? -1 : 1, canTele ? 0.6 : 0, canTele ? 1 : 0,
                    6 / (dist < 3 ? 6 : 1), 0.1, 0.1, 0.1);
			RunicDustMod.spawnParticles(wearer.worldObj, "reddust", Math
