package com.runicdustmod.entity;
...
public class EntityDust extends Entity
{
	...
	/**
	 * The datawatcher is 100% dedicated to the visual aspects of the dust
	 * entity. This is because only the server needs to know anything involving
	 * the logic (the data array is not synced) and the client just needs to
	 * know what the player needs to see.
	 */
	@Override
	...
	public void updateDataWatcher()
	{
		...
		dataWatcher
				.updateObject(DW_starScale, new Float(starScale).byteValue());
		dataWatcher.updateObject(DW_starScaleY,
				new Float(starScaleY).byteValue());
		dataWatcher.updateObject(DW_renderBeam, renderBeam ? (byte) 1
				: (byte) 0);
		dataWatcher.updateObject(DW_beamType, new Integer(beamType));
		dataWatcher.updateObject(DW_renderStar, renderStar ? (byte) 1
				: (byte) 0);
	...
	public void updateEntityFromDataWatcher()
	{
		...
		starScaleY = new Float(
				dataWatcher.getWatchableObjectByte(DW_starScaleY));
	...
	public void setRenderFireOnRuts(boolean b)
	{
		...
		{
			TileEntity tile = worldObj.getBlockTileEntity(i[0], i[1], i[2]);
			if (tile instanceof TileEntityRut)
			{
				TileEntityRut ter = (TileEntityRut) tile;
	...
	{
		return renderFlamesDust;
	}

	public boolean getRenderFireOnRuts()
	{
		return renderFlamesRut;
	}
	...
	protected void readEntityFromNBT(NBTTagCompound tag)
	{
		...
		if (tag.hasKey("beamType"))
			beamType = tag.getInteger("beamType");
	...
	public void onEntityUpdate()
	{
		...
		if (!worldObj.isRemote || true)
		{
			...
			} else if (justBorn)
			{
				...
				for (Integer[] loc : dustPoints)
				{
					if (worldObj.getBlockId(loc[0], loc[1], loc[2]) == DustContent.dust.blockID)
						worldObj.setBlockMetadataWithNotify(loc[0], loc[1],
								loc[2], BlockDust.ACTIVE_DUST, 3);
				}
			}
			if (doFizzle)
			{
				...
				RunicDustMod.spawnParticles(worldObj, "largesmoke",
			...
			if (!this.hasSetTileEntities)
			{
				...
				{
					TileEntityDust ted = (TileEntityDust) worldObj
							.getBlockTileEntity(inPos[0], inPos[1], inPos[2]);
			...
			if (follow)
			{
				...
				if (tf == null)
				{
					EntityPlayer player = worldObj
							.getPlayerEntityByName(summonerUN);
				...
				double dx = cos * dist;
				double dz = sin * dist;
				setPosition(tf.posX + dx, tf.posY + 0.8D + yOffset + 1.2D,
						tf.posZ + dz);
			}
		...
		for (int i = -1; i <= 1; i++)
		{
			for (int k = -1; k <= 1; k++)
			{
				...
				if (blockID == DustContent.dust.blockID
						&& (meta == BlockDust.ACTIVE_DUST || meta == BlockDust.ACTIVATING_DUST))
				{
					TileEntityDust ted = (TileEntityDust) worldObj
							.getBlockTileEntity(x + i, y, z + k);
	...
	/**
	 * Returns true if any dust block under this entity's wing is powered by
	 * redstone
	 * 
	 * @return
	 */
	...
	public void shineRadius(float f, double d, double d1, double d2, int i,
			String s)
	{
		d++;
			...
			for (double d3 = 0.0D; d3 < (Math.PI / 2D); d3 += 0.1D)
			{
				...
				for (int k = 0; k < i; k++)
				{
					worldObj.spawnParticle(
							s,
							((float) posX + f4 + random.nextFloat() * f1) - 0.5F,
							(float) posY + f2,
							((float) posZ + f5 + random.nextFloat() * f1) - 0.5F,
							d, d1, d2);
				}
	...
	public void shineRadiusSphere(float f, double d, double d1, double d2)
	{
		...
		for (double d3 = 0.0D; d3 < (3.14 / 2D); d3 += 0.05)
		{
			...
			for (double d4 = 0.0D; d4 < (3.14 / 2D); d4 += 0.05)
			{
				...
				locations[i + 1] = (double) posY + f3 + f6 + random.nextFloat()
						* f1;
				locations[i + 2] = ((double) posZ + f10 + random.nextFloat()
						* f1) - 0.5D;
				i += 3;

				locations[i] = (double) ((((float) posX - f9) + random
						.nextFloat() * f1) - 0.5F);
				locations[i + 1] = (double) ((float) posY + f3 + f6 + random
						.nextFloat() * f1);
				...
				locations[i + 2] = (double) ((((float) posZ - f10) + random
						.nextFloat() * f1) - 0.5F);
				i += 3;
				RunicDustMod.spawnParticles(worldObj, s, locations, d, d1, d2, 1,
                        0.01, 0.01, 0.01);
			}
		}
	}
	 ...
	 * no ticks are called to the event. Typically used when a rune is finished
	 * and ends
	 */
	public void fade()
	{
