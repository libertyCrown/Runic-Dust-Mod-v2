package com.runicdustmod.block;
...
public class BlockDustTable extends BlockContainer
{
			...
			EntityLivingBase entityliving, ItemStack item)
	{
		...
		{
			((TileEntityFurnace) world.getBlockTileEntity(i, j, k))
	...
	public boolean onBlockActivated(World world, int i, int j, int k,
			EntityPlayer player, int dir, float x, float y, float z)
	{
		if (/* world.multiplayerWorld */false)
		{
			return true;
		} else if (player.getCurrentEquippedItem() != null
				&& player.getCurrentEquippedItem().itemID == DustContent.runicPaper.itemID)
		{
			int page = (((TileEntityDustTable) world
					.getBlockTileEntity(i, j, k)).page - 1);
		...
		} else
		{
			...
			TileEntityDustTable tedt = (TileEntityDustTable) world
					.getBlockTileEntity(i, j, k);
			tedt.page--;

			if (tedt.page < 0)
			{
				tedt.page = DustManager.getNames().size() - RunicDustMod.numSec;
			}
			...
			EntityPlayer entityplayer)
	{
		...
		} else
		{
			...
			if (tedt.page >= DustManager.getNames().size() - RunicDustMod.numSec + 1)
			{
				tedt.page = 0;
			}
		}
	...
	{
		this.blockIcon = par1IconRegister.registerIcon(References.spritePath
				+ "table_side");
		this.topTex = par1IconRegister.registerIcon(References.spritePath
				+ "table_top");
		this.botTex = par1IconRegister.registerIcon(References.spritePath
				+ "table_bottom");
	}
}
